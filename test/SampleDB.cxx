 ...
 *
 * This database is a tweaked copy of the MySQL sample database at
...
void
SampleDB::init(
        const wr::u8string_view &uri
)
{
        open(uri);

        if (!hasObject("table", "customers")) {
                createSchema();
                populateAllTables();
        }
}
...
void
SampleDB::createSchema(
        Session &db
) // static
{
        db.exec(R"(CREATE TABLE IF NOT EXISTS product_lines (
        ...
        db.exec(R"(CREATE TABLE IF NOT EXISTS products (
                ...
                CONSTRAINT products_ibfk_1 FOREIGN KEY (line)
                                           REFERENCES product_lines(name)
        );)");

        db.exec(R"(CREATE TABLE IF NOT EXISTS offices (
        ...
        db.exec(R"(CREATE TABLE IF NOT EXISTS employees (
                ...
                CONSTRAINT employee_ibfk_1 FOREIGN KEY (reports_to)
                                           REFERENCES employees(number),
                CONSTRAINT employee_ibfk_2 FOREIGN KEY (office_code)
                                           REFERENCES offices(code)
        );)");

        db.exec(R"(CREATE TABLE IF NOT EXISTS customers (
                number                INTEGER PRIMARY KEY NOT NULL,
                name                  VARCHAR(50) NOT NULL,
                contact_surname       VARCHAR(50) NOT NULL,
                contact_forename      VARCHAR(50) NOT NULL,
                phone                 VARCHAR(50) NOT NULL,
                address_line_1        VARCHAR(50) NOT NULL,
                address_line_2        VARCHAR(50) DEFAULT NULL,
                city                  VARCHAR(50) NOT NULL,
                ...
                country               VARCHAR(50) NOT NULL,
                sales_rep_employee_no INTEGER DEFAULT NULL,
                credit_limit          DECIMAL(10, 2) DEFAULT NULL,

                CONSTRAINT customer_ibfk_1 FOREIGN KEY (sales_rep_employee_no)
                                           REFERENCES employees(number)
        );)");

        db.exec(R"(CREATE TABLE IF NOT EXISTS payments (
                ...
                CONSTRAINT payments_ibfk_1 FOREIGN KEY (customer_no)
                                           REFERENCES customers(number)
                                           ON DELETE CASCADE
        );)");

        db.exec(R"(CREATE TABLE IF NOT EXISTS orders (
                number        INTEGER PRIMARY KEY NOT NULL,
                date          DATE NOT NULL,
                ...
                CONSTRAINT orders_ibfk_1 FOREIGN KEY (customer_no)
                                         REFERENCES customers(number)
                                         ON DELETE CASCADE
        );)");

        db.exec(R"(CREATE TABLE IF NOT EXISTS order_details (
                ...
                CONSTRAINT order_details_ibfk_1 FOREIGN KEY (order_no)
                                                REFERENCES orders(number)
                                                ON DELETE CASCADE,
                CONSTRAINT order_details_ibfk_2 FOREIGN KEY (product_code)
...
) // static
{
        ...
        db.exec("DROP TABLE IF EXISTS product_lines");
}

//--------------------------------------

void
SampleDB::populateCustomers(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO customers ("
                        "number, name, contact_surname, contact_forename, "
                        "phone, address_line_1, address_line_2, city, state, "
                        "postal_code, country, sales_rep_employee_no, "
                        "credit_limit) VALUES "
...
void
SampleDB::populateOffices(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO offices ("
                        "code, city, phone, address_line_1, address_line_2, "
                        "state, country, postal_code, territory) VALUES "
...
void
SampleDB::populateEmployees(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO employees ("
                        "number, surname, forename, extension, email, "
                        "office_code, reports_to, job_title) VALUES "
...
void
SampleDB::populateProductLines(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO product_lines ("
                      "name, text_description, html_description, image) VALUES "
...
void
SampleDB::populateProducts(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO products ("
                        "code, name, line, scale, vendor, description, "
                        "quantity_in_stock, buy_price, msrp) VALUES "
...
void
SampleDB::populateOrders(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO orders ("
                        "number, date, required_date, shipped_date, status, "
                        "comments, customer_no) VALUES "
...
void
SampleDB::populateOrderDetails(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO order_details ("
                        "order_no, product_code, quantity, unit_price, "
                        "order_line_no) VALUES "
...
void
SampleDB::populatePayments(
        Session &db
) // static
{
        db.exec(u8"INSERT INTO payments ("
                        "customer_no, cheque_no, date, amount) VALUES "
...
) // static
{
        ...
        populateProductLines(db);
        populateProducts(db);
        populateOrders(db);
        populateOrderDetails(db);
        populatePayments(db);
}
